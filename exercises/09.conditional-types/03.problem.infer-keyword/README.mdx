...
👨‍💼 Now that you understand basic `infer`, let's use it for more complex
...
```ts
// [...infer Rest, infer Last] matches a tuple, capturing all-but-last and last
type LastElement<T> = T extends [...infer _, infer L] ? L : never
```
