...
await testStep('displayUser function displays user information', () => {
	...
	expect(result, '🚨 result should be "User: Test User (test@example.com)" - import User type and use it correctly').toBe('User: Test User (test@example.com)')
})
...
await testStep('displayProduct function displays product information', () => {
	...
	expect(result, '🚨 result should be "Product: Test Product - $99.99" - import Product type and use it correctly').toBe('Product: Test Product - $99.99')
})
